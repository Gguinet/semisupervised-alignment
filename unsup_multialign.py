...
parser.add_argument('--maxload', default=10000, type=int, help='Max number of loaded vectors')
...
def g_wasserstein(x_src, x_tgt, C2):
    N = x_src.shape[0]
    C1 = GWmatrix(x_src)
    M = gromov_wasserstein(C1,C2,np.ones(N),np.ones(N),'square_loss')#epsilon=0.55,max_iter=100,tol=1e-4
