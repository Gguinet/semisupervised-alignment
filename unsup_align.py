...
parser.add_argument(
    "--niter", default=5000, type=int, help="Initial number of iterations"
)
parser.add_argument("--bsz", default=500, type=int, help="Initial batch size")
parser.add_argument("--lr", default=500.0, type=float, help="Learning rate")
parser.add_argument(
    "--nmax", default=10000, type=int, help="Vocabulary size for learning the alignment"
)
parser.add_argument(
...
def align(
    X, Y, R, lr=10.0, bsz=200, nepoch=5, niter=1000, nmax=10000, reg=0.05, verbose=True
):
...
w_src, x_src = load_vectors(
    args.model_src, maxload=args.maxload, norm=True, center=True
)
w_tgt, x_tgt = load_vectors(
    args.model_tgt, maxload=args.maxload, norm=True, center=True
)
